...
divReduce [] = []
divReduce (s1@(Div (id1, classes1, kvs1) ils1) :
            s2@(Div (id2, classes2, kvs2) ils2) :
            ils) =
  let classes'  = classes1 `intersect` classes2
      kvs'      = kvs1 `intersect` kvs2
      classes1' = classes1 \\ classes'
  ...
  in
   case null classes' && null kvs' of
     True -> s1 : (divReduce (s2 : ils))
     False -> let attr'  = ("", classes', kvs')
                  attr1' = (id1, classes1', kvs1')
                  attr2' = (id2, classes2', kvs2')
              in
               divReduce (Div attr' [(Div attr1' ils1), (Div attr2' ils2)] :
                           ils)
divReduce (il:ils) = il : (divReduce ils)

foo :: Pandoc -> Pandoc
foo = (bottomUp divReduce) . (bottomUp spanReduce)
